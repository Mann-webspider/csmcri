...
export async function sendResetEmail(toEmail:string,resetToken:string): Promise<string> {
  const resetLink:string = `${config.front_app_url}/auth/password-reset?token=${resetToken}`
  // send mail with defined transport object
  const info = await transporter.sendMail({
    from: '"CSMCRI 😃"', // sender address
